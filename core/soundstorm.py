...
class ConformerWrapper(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer('quantizer_offsets', torch.arange(num_effective_quantizers) * num_codes_with_mask,
                             persistent=False)
...
class SoundStorm(nn.Module):
    ...
    def generate(self,
        ...
        if exists(prompt_tokens):
            ...
            mask = torch.cat([torch.zeros_like(prompt_acoustic_tokens, dtype=torch.bool, device=device), mask.bool()],
                             axis=1)
        else:
            mask = mask.bool()
        ...
        for q in range(self.num_quantizers):
            all_mask_num_tokens = all_mask_num_tokens if q < num_full_sampling_levels else torch.zeros((1, batch_size),
                                                                                                       dtype=torch.long,
                                                                                                       device=device)
