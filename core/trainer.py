...
class SoundStormTrainer(nn.Module):
    ...
    def __init__(
        ...
        if exists(trainset):
            ...
            self.ds = SoundStormDataset(file_list=train_file_list,
                                        ...
                                        max_sequence=int(
                                            max_sequence * self.downsample_rate) if is_raw_wav else max_sequence,
                                        **tokenizer_kwargs)
        if exists(devset):
        ...
        assert len(
            self.valid_ds) >= batch_size, f'validation dataset must have sufficient number of samples (currently {len(self.valid_ds)}) for training'

        # dataloader

        self.dl = get_dataloader(self.ds, is_raw_wav=is_raw_wav, batch_size=batch_size, shuffle=True,
                                 drop_last=drop_last, num_workers=num_workers)
        self.valid_dl = get_dataloader(self.valid_ds, is_raw_wav=is_raw_wav, batch_size=batch_size, shuffle=False,
        ...
        hps = {"num_train_steps": num_train_steps, "num_warmup_steps": num_warmup_steps, "learning_rate": lr,
               "initial_learning_rate": lr, "epochs": epochs}
    ...
    def tokenize(self, batch):
        ...
        tmp_model = self.model.module if isinstance(self.model,
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for batch in self.dl:
                ...
                accum_log(logs,
                          {'loss': loss.item() / self.grad_accum_every, 'acc': acc.item() / self.grad_accum_every})
                ...
                if grad_accum == self.grad_accum_every:
                    ...
                    if self.is_main and not (steps % self.log_steps):
                        ...
                        self.accelerator.log(
                            {"train/loss": logs['loss'], "train/acc": logs['acc'], "train/learning_rate": lr},
                            step=steps)
                    logs = {}
